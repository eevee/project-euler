{- Problem 67: Using an efficient algorithm find the maximal sum in the
 - triangle?
 -
...
-- Apologies but there's no way I'm going to bother parsing the provided file
triangle = [ [59],
             [73, 41],
             [52, 40, 09],
             [26, 53, 06, 34],
...
-- Simple function for finding the max element in a list
listMax :: [Integer] -> Integer
listMax []     = 0
listMax (a:[]) = a
listMax (a:b)  = max a (listMax b)

-- Heart of the matter.  This collapses two arrays of integers into one, by
-- increasing every element of the second by the higher of either the element
-- in the first with the same index or the one immediately following.  That,
-- this:
--     0 2 4 6
--      8 5 9 3
-- becomes simply:
--     10 9 15 9
-- Note that you have to prepend the implied zero, so the two provided arrays
-- are the same size.
collapseTwoRows :: [Integer] -> [Integer] -> [Integer]
collapseTwoRows (a:[]) (d:[]) = [a + d]
collapseTwoRows (a:(b:c)) (d:e) = ((max a b) + d) : (collapseTwoRows (b:c) e)
...
main = putStrLn ((show . listMax . collapsedTriangle) triangle)
